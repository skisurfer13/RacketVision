...
class BounceDetector:
    ...
    def detect_bounce_positions(self, ball_detections):
        x_ball = []
        y_ball = []
        for position in ball_detections:
            x_ball.append(position[1][0])
            y_ball.append(position[1][1])

        bounce_frames = self.predict(x_ball, y_ball)
        bounce_frames = list(bounce_frames)
        bounce_frames.sort()
